...
namespace WorldWizards.core.entity.coordinate.utils
{
	...
	public static class CoordinateHelper
    {
        ...
        public static Coordinate convertUnityCoordinateToWWCoordinate(Vector3 coordinate, int rotation)
        {
            var full = coordinate / GetTileScale();
            var fraction = full - new Vector3(Mathf.Floor(full.x), Mathf.Floor(full.y), Mathf.Floor(full.z));
        ...
        {
            var offsetX = coordinate.offset.x * GetTileScale() * 0.5f;
            var offsetY = coordinate.offset.y * GetTileScale() * 0.5f;
            var offsetZ = coordinate.offset.z * GetTileScale() * 0.5f;
            var offset = new Vector3(offsetX, offsetY, offsetZ);
            var c = new Vector3(coordinate.index.x, coordinate.index.y, coordinate.index.z) * GetTileScale();
            return c + offset;
        }
    }
}